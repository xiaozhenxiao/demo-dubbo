 ...
 * @author wangzhen
 * @version 1.0
 * @date 2016-10-08 14:07
 **/
public class AllHashFunction implements HashFunction {
    /**
     * 加法hash
     *
     * @param key   字符串
     ...
     */
    @Override
    public int additiveHash(String key, int prime) {
        int hash, i;
     ...
     * @return 结果hash
     */
    public int bernstein(String key) {
        int hash = 0;
        int i;
     ...
     * Universal Hashing
     */
    public int universal(char[] key, int mask, int[] tab) {
     ...
     * Zobrist Hashing
     */
    public int zobrist(char[] key, int mask, int[][] tab) {
        int hash, i;
        for (hash = key.length, i = 0; i < key.length; ++i)
            hash ^= tab[i][key[i]];
        return (hash & mask);
